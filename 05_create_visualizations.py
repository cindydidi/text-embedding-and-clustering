#!/usr/bin/env python3
"""
Step 5: Create reports and visualizations for cluster analysis.

Default mode (no flags):
- Produces a fixed set of outputs that combines the old Step 05 (comparison visuals)
  (overall summary + comparison charts).
...
def parse_chi_square_from_report(report_path=COMPARISON_REPORT):
    """Parse chi-square statistic and p-value from step 4 report."""
    if not os.path.isfile(report_path):
        return None, None
    try:
        with open(report_path, 'r', encoding='utf-8') as f:
...
def write_overview_report(
    ...
    if chi2 is not None and chi2_p is not None:
        lines.append("")
        lines.append("Step 4 chi-square (clusters × groups):")
...
def llm_mode_interactive(confirm_before_run=False):
    ...
    if not dfs:
        raise FileNotFoundError("No input CSVs found. Run steps 03/04 first (and keep outputs in this folder).")

    # Gemini setup (reuse step 03 API key)
    try:
    ...
    if not api_key:
        raise RuntimeError("GOOGLE_API_KEY not found. Put it in .env as GOOGLE_API_KEY=... (same as step 03).")
...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(
            description="Step 5: Create cluster reports and visualizations (default), or run interactive LLM mode (--llm)."
        )
        parser.add_argument(
            "--llm",
            action="store_true",
