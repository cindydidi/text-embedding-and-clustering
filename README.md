...
This project was vibe coded. The author doesn’t know a single line of code. There could be mistakes, redundancy, or places where the style isn’t exactly graceful. If you have suggestions, feel free to open a pull request.
...
这个 repo 是 vibe code 来的，本文科生是一行代码也看不懂，如有错误，redundancy 或者 style 不好的你就忍了吧，别骂我。谢谢。欢迎 pull request。
...
### 遇到问题怎么办
- **出现「GOOGLE_API_KEY not found」** — 在这个文件夹里加一个 `.env` 文件，写上 `GOOGLE_API_KEY=你的密钥`。
- **少装了依赖** — 再执行一次 `pip install -r requirements.txt`。
