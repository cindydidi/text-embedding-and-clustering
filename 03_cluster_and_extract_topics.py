#!/usr/bin/env python3
"""
Step 3: Perform K-means clustering and extract topic labels from clusters
(FIXED: dimensionality reduction + LLM-based topic labels)
"""
...
def load_data(embeddings_file, metadata_file, optimal_k_file, manual_k=None):
    """Load embeddings, metadata, and optimal k."""
    print("Loading data...")
...
def generate_topic_label_llm(messages, cluster_id):
    """
    Generate topic label using Gemini LLM API.
    ...
    if genai is not None:
        try:
            ...
            if genai._configured:
                ...
                prompt = f"""Generate a concise topic label (2-4 words) for these customer service messages. 
The label should describe the main theme or topic.

Messages:
{messages_text}

Topic label (2-4 words only):"""
    ...
    return generate_topic_label_fallback(messages, cluster_id)
...
def generate_topic_label_fallback(messages, cluster_id):
    """Fallback topic label generation using simple keyword extraction."""
    keywords = extract_keywords_simple(messages)
    if keywords:
...
def analyze_clusters(cluster_labels, metadata, use_llm=True):
    """Analyze clusters and extract topics."""
    print("\nAnalyzing clusters and extracting topics...")
    ...
    n_noise = (cluster_labels == -1).sum()
    if n_noise > 0:
        print(f"  Note: {n_noise:,} messages marked as noise (outliers) will be excluded from topic analysis")
    ...
    for idx, cluster_id in enumerate(unique_clusters):
        ...
        if use_llm:
            topic_label, top_keywords = generate_topic_label_llm(cluster_messages, cluster_id)
        else:
            topic_label, top_keywords = generate_topic_label_fallback(cluster_messages, cluster_id)
        
        # Ensure label is unique
        original_label = topic_label
        counter = 1
        while topic_label.lower() in used_labels:
            topic_label = f"{original_label} ({counter})"
            counter += 1
        
        used_labels.add(topic_label.lower())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Cluster embeddings and extract topic labels')
    ...
    parser.add_argument('--dimensions', type=int, default=250,
                        help='Number of dimensions for PCA reduction (default: 250, higher preserves more topic distinction)')
    ...
    output_file = "clusters_with_topics.csv"
    
    # Load data
    # For HDBSCAN, k is not used, but we still need it for other algorithms
    embeddings, metadata, optimal_k = load_data(
        embeddings_file, metadata_file, optimal_k_file, manual_k=args.k
    )
